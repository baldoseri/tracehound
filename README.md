...
*Real output, not a mockup — the animation is generated by `make demo-gif`, which
re-runs the sensor and redraws whatever it printed, so it cannot drift out of date.*
...
- **IP fragments are not reassembled.** IPv6 extension header chains *are* walked, so a
  first fragment decodes normally; a non-initial fragment carries no transport header
  and is counted as undecodable rather than misparsed.
- **The public suffix table is partial.** Around sixty common two-label suffixes are
  built in, so `a.example.co.uk` correctly groups under `example.co.uk`. The full
  Public Suffix List would be exhaustive at the cost of a megabyte of embedded data and
  a standing update obligation, for a detector whose scoring is dominated by entropy.
- **Alerts repeat as evidence grows.** Identical evidence is suppressed outright and an
  escalation in severity is reported immediately, but a finding whose numbers keep
  climbing will be restated once per cooldown. Tune with `-min-severity`.
